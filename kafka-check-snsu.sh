...
echo ""
echo "===== Kafka 브로커 9094 포트 확인 ====="
if [ -z "${#BROKER_PODS[@]}" ]; then
    echo "kafka 브로커 파드가 없습니다."
    exit 1
fi
# 각 파드에서 netstat 실행
for POD in ${BROKER_PODS[@]}; do
    echo "Checking pod: $POD"
    oc exec -n $NAMESPACE $POD -- netstat -antp | grep 9094 || echo "포트 9094가 LISTEN 상태가 아닙니다."
    echo "--------------------------------------------------"
done
...
function exec_on_working_broker() {
    ...
    for BROKER in "${BROKER_PODS[@]}"; do
        ...
        # Kafka 명령 실행
        OUTPUT=$(oc exec -n $NAMESPACE "$BROKER" -- bash -c "$CMD" 2>&1)
        STATUS=$?

        # Kafka 명령이 실행은 되었지만 내부적으로 실패했는지 확인
        if [[ $STATUS -eq 0 && ! "$OUTPUT" =~ "Exception" && ! "$OUTPUT" =~ "Error" && ! "$OUTPUT" =~ "Caused by" ]]; then
            echo "$OUTPUT"
            return 0
        else
            echo "[실패] $BROKER: 명령 실패 또는 Kafka 내부 오류 발생" >&2
            echo "$OUTPUT" >&2
        fi
    done
    echo "모든 브로커에서 명령 실행 실패!" >&2
    return 1
}
