...
check_ai() {
    ...
    PRED_LG=$(oc exec -n nwdaf "${CH_POD:-clickhouse-shard0-0}" -- bash -c 'clickhouse-client --password ${CLICKHOUSE_ADMIN_PASSWORD} -d nwdaf -q "SELECT window_end, model_type, count() FROM ai.t_cell_prb_usage_predicted WHERE model_type IN ('\''lstm'\'','\''gru'\'') GROUP BY window_end, model_type ORDER BY window_end DESC LIMIT 30"' 2>/dev/null || true)
    # window_end가 날짜+시간으로 분리되어 4개 필드로 출력됨: 날짜 시간 model_type count
    ...
    PRED_LG_LATEST_DT=$(echo "$PRED_LG" | awk 'NR==1 && NF>=4 {print $1" "$2}')
    PRED_LG_RECENT=0
    if [[ -n "$PRED_LG_LATEST_DT" ]]; then
        PRED_LG_LATEST_TS=$(TZ=UTC date --date="$PRED_LG_LATEST_DT" +%s 2>/dev/null || echo 0)
        [[ $PRED_LG_LATEST_TS -ge $FIVE_MIN_AGO ]] && PRED_LG_RECENT=1
    fi
    if [[ "$PRED_LG_SUM" -ge 1 && "$PRED_LG_ISSUES_CNT" -eq 0 && "$PRED_LG_RECENT" -eq 1 ]]; then 
        report "AI LSTM/GRU Infer Result(Old)" "OK" "최근 5분 내 적재 확인"
    elif [[ "$PRED_LG_SUM" -ge 1 && "$PRED_LG_ISSUES_CNT" -eq 0 ]]; then
        report "AI LSTM/GRU Infer Result(Old)" "WARNING" "5분 내 신규 적재 없음 (최근: $PRED_LG_LATEST_DT)"
    elif [[ "$PRED_LG_SUM" -ge 1 && "$PRED_LG_ISSUES_CNT" -gt 0 ]]; then 
        report "AI LSTM/GRU Infer Result(Old)" "WARNING" "모델 충돌 발견"
    else 
        report "AI LSTM/GRU Infer Result(Old)" "INFO" "lstm/gru 미적재 (참고)"
    fi
    if [[ -n "${PRED_LG//[[:space:]]/}" ]]; then echo "$PRED_LG" | sed 's/^/  /'; fi

    echo ""
    echo "[3] 추론용 데이터셋 적재 상태 체크"
    INFER_DATA=$(oc exec -n nwdaf "${CH_POD:-clickhouse-shard0-0}" -- bash -c 'clickhouse-client --password ${CLICKHOUSE_ADMIN_PASSWORD} -d nwdaf -q "SELECT window_end, count() FROM ai.cell_usage_1m_latest WHERE window_end >= now() - toIntervalMinute(2) GROUP BY 1 ORDER BY 1 DESC"' 2>/dev/null || true)
    INFER_ROWS=$(echo "$INFER_DATA" | sed '/^[[:space:]]*$/d' | wc -l | tr -d ' ')
    if [[ -n "${INFER_DATA//[[:space:]]/}" && "$INFER_ROWS" -ge 1 ]]; then 
        report "AI Inference Dataset(Old)" "OK" "최근 2분 내 데이터 확인"
        echo "$INFER_DATA" | sed 's/^/  /'
    else 
        report "AI Inference Dataset(Old)" "CRITICAL" "최근 2분 내 데이터 없음"
    fi

    INFER_DATA2=$(oc exec -n nwdaf "${CH_POD:-clickhouse-shard0-0}" -- bash -c 'clickhouse-client --password ${CLICKHOUSE_ADMIN_PASSWORD} -d nwdaf -q "SELECT window_end, count() FROM ai.cell_usage_infer WHERE window_end >= now() - toIntervalMinute(2) GROUP BY 1 ORDER BY 1 DESC"' 2>/dev/null || true)
    INFER_ROWS2=$(echo "$INFER_DATA2" | sed '/^[[:space:]]*$/d' | wc -l | tr -d ' ')
    if [[ -n "${INFER_DATA2//[[:space:]]/}" && "$INFER_ROWS2" -ge 1 ]]; then
        report "AI InferDS_1m(new)" "OK" "최근 2분 내 데이터 확인"
        echo "$INFER_DATA2" | sed 's/^/  /'
    else
        report "AI InferDS_1m(new)" "CRITICAL" "최근 2분 내 데이터 없음"
    fi

    echo ""
    echo "[4] 학습용 데이터셋 적재 상태 체크"
    TRAIN_DATA=$(oc exec -n nwdaf "${CH_POD:-clickhouse-shard0-0}" -- bash -c 'clickhouse-client --password ${CLICKHOUSE_ADMIN_PASSWORD} -d nwdaf -q "SELECT window_end, count() FROM ai.cell_usage_hist_5m WHERE window_end >= now() - toIntervalMinute(70) GROUP BY 1 ORDER BY 1 DESC LIMIT 5"' 2>/dev/null || true)
    TRAIN_ROWS=$(echo "$TRAIN_DATA" | sed '/^[[:space:]]*$/d' | wc -l | tr -d ' ')
    if [[ -n "${TRAIN_DATA//[[:space:]]/}" && "$TRAIN_ROWS" -ge 1 ]]; then 
        report "AI Training Dataset(Old)" "OK" "학습 데이터 적재 확인"
    else 
        report "AI Training Dataset(Old)" "CRITICAL" "학습 데이터 없음"
    fi

    TRAIN_INFER_DATA=$(oc exec -n nwdaf "${CH_POD:-clickhouse-shard0-0}" -- bash -c 'clickhouse-client --password ${CLICKHOUSE_ADMIN_PASSWORD} -d nwdaf -q "SELECT window_end, cell_type, count() FROM ai.cell_usage_infer_5m GROUP BY 1, 2 ORDER BY 1 DESC, 2 DESC LIMIT 5"' 2>/dev/null || true)
    ...
    if [[ -n "${TRAIN_INFER_DATA//[[:space:]]/}" && "$TRAIN_INFER_ROWS" -ge 1 ]]; then
        ...
        if [[ "$IS_RECENT" -eq 1 && "$HAS_LTE" -eq 1 && "$HAS_5G" -eq 1 ]]; then
            report "AI TrainDS(infer_5m)" "OK" "80분 내 lte/5g 데이터 확인"
        else
            ti_detail=""
            ...
            report "AI TrainDS(infer_5m)" "CRITICAL" "$ti_detail"
        fi
        echo "$TRAIN_INFER_DATA" | sed 's/^/  /'
    else
        report "AI TrainDS(infer_5m)" "CRITICAL" "데이터 없음"
    fi
    ...
    local model_status=""
    if [[ -z "${lstm_files//[[:space:]]/}" && -z "${gru_files//[[:space:]]/}" ]]; then
        report "MinIO Model Files" "CRITICAL" "LSTM/GRU 파일 없음"
    elif [[ -n "${lstm_files//[[:space:]]/}" && -n "${gru_files//[[:space:]]/}" ]]; then
        report "MinIO Model Files" "OK" "LSTM/GRU 파일 존재"
    else
        if [[ -z "${lstm_files//[[:space:]]/}" ]]; then
            echo "=> LSTM 모델 파일 미존재 [CRITICAL]"
            model_status="LSTM 파일 없음"
        else
            echo "=> LSTM 모델 파일 존재 확인 [OK]"
        fi
        if [[ -z "${gru_files//[[:space:]]/}" ]]; then
